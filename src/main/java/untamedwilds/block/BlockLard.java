...
public class BlockLard extends BreakableBlock {
    ...
    public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
        if (this.method_23356(pos, entity)) {
            Vec3d vec3d_1 = entity.getMotion();
            if (vec3d_1.y < -0.05D) {
                entity.setMotion(new Vec3d(vec3d_1.x, -0.05D, vec3d_1.z));
            }
    ...
    private boolean method_23356(BlockPos blockPos_1, Entity entity_1) {
        if (entity_1.onGround) {
            return false;
        } else if (entity_1.getPosition().getY() > (double)blockPos_1.getY() + 0.9375D - 1.0E-7D) {
            return false;
    ...
    private void spawnParticleEffects(Entity entity, World world, BlockPos blockPos_1, int int_1, double double_1, double double_2, double double_3, double double_4, double double_5, double double_6) {
        ...
        for(int int_2 = 0; int_2 < int_1; ++int_2) {
            entity.world.addParticle(new BlockParticleData(ParticleTypes.BLOCK, blockState_1), entity.getPosition().getX() + double_1, entity.getPosition().getY() + double_2, entity.getPosition().getZ() + double_3, double_4, double_5, double_6);
        }
    }
}
