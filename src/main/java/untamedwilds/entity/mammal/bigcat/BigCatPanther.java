...
public class BigCatPanther extends BigCatAbstract {
    ...
    public BigCatPanther(EntityType<? extends BigCatAbstract> type, World worldIn) {
        super(type, worldIn);
        this.ecoLevel = 7;
    }
    ...
    public void breed() {
        for (int i = 0; i <= 1 + this.rand.nextInt(2); i++) {
            BigCatPanther child = this.createChild(this);
            if (child != null) {
    ...
    public BigCatPanther createChild(AgeableEntity ageable) {
        BigCatPanther bear = new BigCatPanther(ModEntity.PANTHER, this.world);
